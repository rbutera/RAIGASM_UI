...
LookingForGroup_Options:RegisterEvent("LFG_UPDATE")
		...
		control.SetItemValue = function(self,key)
			...
			if IsLFGDungeonJoinable(id) then
				...
				if numCompletions <= 0 then
					check:SetLabel(shortage and table.concat{"|cff0000ff",name} or name)
				else
					check:SetLabel(table.concat{shortage and "|cffff00ff" or "|cff00ff00" ,name,"|r"})
				end
			else
				check:SetLabel(table.concat{"|cffcccccc",name,"|r"})
			end
			if raidname ~= "" then
				if raidname == name then
					check:SetDescription(table.concat{"|cff8080cc",_G["PLAYER_DIFFICULTY"..subtypeID],"|r"})
				else
					check:SetDescription(table.concat{"|cff8080cc",raidname,"|r"})
				end
			else
			...
			check:SetCallback("OnEnter", function(self,...)
				...
				if moneyAmount ~= 0 then
					...
					GameTooltip:AddDoubleLine(MONEY,table.concat{g,"|TInterface\\MoneyFrame\\UI-GoldIcon:12:12|t",
																math.floor(c/100),"|TInterface\\MoneyFrame\\UI-SilverIcon:12:12|t",
																c%100,"|TInterface\\MoneyFrame\\UI-CopperIcon:12:12|t"},0.5,0.5,0.8,true)
				end
				...
				for i = 1,LFG_ROLE_NUM_SHORTAGE_TYPES do
					...
					if itemCount ~= 0 or money ~= 0 or xp ~= 0 then
						local t = {}
						if forTank then
							t[#t+1] = "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES:16:16:0:0:64:64:0:19:22:41|t"
						end
						if forHealer then
							t[#t+1] = "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES:16:16:0:0:64:64:20:39:1:20|t"
						end
						if forDamage then
							t[#t+1] = "|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES:16:16:0:0:64:64:20:39:22:41|t"
						end
						if #t ~= 0 then
...
local rf_tb = {}
LookingForGroup_Options:push("lfd",
{
	...
	args=
	{
		rf =
		{
			...
			values = function()
				...
				local all_dg = LookingForGroup_Options.db.profile.lfdrf_all_dg			
		...
		sall =
		{
			...
			order = -1,
			set = function(_,val)
				LookingForGroup_Options.db.profile.lfdrf_all_dg = val or nil
			end,
			get = function()
				return LookingForGroup_Options.db.profile.lfdrf_all_dg
			end,
		}
	}
})
