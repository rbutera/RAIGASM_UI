...
function LookingForGroup_Options.updateapplicant(obj)
	...
	local id, status, pendingStatus, numMembers, isNew, comment = C_LFGList.GetApplicantInfo(users.val)
...
--				obj:ToggleChecked()
			...
			local id, status, pendingStatus, numMembers, isNew, comment = C_LFGList.GetApplicantInfo(user.val)
			if numMembers == 1 then
				EasyMenu(GetApplicantMemberMenu(id,1), LFGListFrameDropDown, "cursor" , 0, 0, "MENU")
			else
				...
				EasyMenu(GetApplicantMemberMenu(id,pos), LFGListFrameDropDown, "cursor" , 0, 0, "MENU")
			end
		end
	end)
	...
	frame:SetScript("OnEnter", function(self,...)
		...
		LookingForGroup_Options:applicants_tooltip()
	end)
	check.width = "fill"
